...
from minigpt4.models.modeling_mt5 import MT5ForConditionalGeneration
...
class MiniGPT4(Blip2Base):
    ...
    def __init__(
        ...
        self.use_artelingo = use_artelingo
        ...
        self.llama_tokenizer = AutoTokenizer.from_pretrained(checkpoint)
        ...
        if self.low_resource:
            ...
            print('low resource model used')
        else:
            # self.llama_model = LlamaForCausalLM.from_pretrained(
            self.llama_model = AutoModelForCausalLM.from_pretrained(
            # self.llama_model = MT5ForConditionalGeneration.from_pretrained(
                llama_model,
                torch_dtype=torch.bfloat16,
                # torch_dtype=torch.bfloat16,
                # device_map="auto"
        ...
        # torch.nn.init.uniform_(self.llama_proj.weight, -5, 5)
    ...
    # def prompt_wrap(self, img_embeds, atts_img, prompt, language=None, emotion=None):
    #     if prompt:
    ...
    #         # p_before_embeds = self.llama_model.encoder.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1, -1) # mt0
    #         # p_after_embeds = self.llama_model.encoder.embed_tokens(p_after_tokens.input_ids).expand(batch_size, -1, -1)
    #         # p_before_embeds = self.llama_model.model.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1, -1) # llama
    ...
    #                 instructions = [f"###Instruction: Use only {l} characters to describe the image. Make the caption reflect {e} emotion. " for l, e in zip(language, emotion)]
    #             else:
    #                 instructions = [f"###Instruction: Use only {l} characters to describe the image. " for l in language]
    ...
    #             wrapped_img_embeds = torch.cat([instruction_embeds, p_before_embeds, img_embeds, p_after_embeds], dim=1)
    #         else:
    ...
    def forward(self, samples):
        ...
        # to_regress_embeds = self.llama_model.encoder.embed_tokens(to_regress_tokens.input_ids)
        ...
        # inputs_embeds = img_embeds.to(torch.bfloat16)
    ...
    def from_config(cls, cfg):
        ...
        use_artelingo = cfg.get("use_artelingo", False)
