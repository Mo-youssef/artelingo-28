...
#         cap = llama_tokenizer.tokenize(cap)
#     else:
#         print(f"Warning: Unable to tokenize non-string value: {cap}")
#         cap = ""
#     cap = ' '.join(list(cap))
#     return cap
...
def main():
    ...
    for language in generations_all['language'].unique():
        print(f'================== {language} ==================')
        references = references_all[references_all['language'] == language]
        grouped_references = group_references(references)
        generations = generations_all[generations_all['language'] == language]
        print('Done\nMerging')
        # pdb.set_trace()
            ...
            try:
                if (m == 'bertscore'):
                    if (language in ['english']):
                        scores_all = metr.compute(predictions=hypothesis, references=references, lang='en')
                        scores = {k: np.mean(v) for k, v in list(scores_all.items())[:3]}
                    else: continue
                elif m == 'cider':
                ...
                elif (m in ['meteor']): #and (language in ['chinese', 'arabic']):
